...
<p style="color:#333333;font-size:16px;font-weight:bold;margin-left:25px;margin-top:10px;">My Notices

<span style="color:#666666;font-size:12px;font-weight:bold;margin-right:25px;margin-top:10px;float:right"><a href="home-page.php" style="color:#999999">Home</a> &raquo My Notices</span>
</p>
	...
	$curr_state=db_scalar("select notice_is_imp from tbl_notice where 1 and notice_id='$_REQUEST[nid]'");
	
	if($curr_state=="Yes"){
	db_query("update tbl_notice set notice_is_imp='No' where 1 and notice_id='$_REQUEST[nid]'");	
	}else{
	db_query("update tbl_notice set notice_is_imp='Yes' where 1 and notice_id='$_REQUEST[nid]'");	
	}
    

	
	
	header("location:notice-board-member.php?pgid=mi&mty=in");
	exit;	
	}
	
	if(isset($_REQUEST['MakeUnread'])){
	db_query("update tbl_notice set notice_msg_status='Unread' where 1 and notice_id='$_REQUEST[noticeid]'");	
	header("location:notice-board-member.php?pgid=mi&mty=in");
	exit;
	}
	
	if(isset($_REQUEST['DetailDelete'])){
	db_query("update tbl_notice set notice_is_delete='Yes' where 1 and notice_id='$_REQUEST[noticeid]'");	
	header("location:notice-board-member.php?pgid=mi&mty=in");
	exit;
	}
	...
	db_query("update tbl_notice set notice_msg_status='Read' where 1 and notice_id='$_REQUEST[noticeid]'");
	
	$sql="select * from  tbl_notice where 1 and notice_id='$_REQUEST[noticeid]'";
	$rec_guide=mysql_fetch_array(db_query($sql));	
	
	?>
<form action="" method="post" >
<div> <a href="notice-board-member.php?pgid=mi&mty=in">
...
<input type="hidden" name="notice_id_h" value="<?=$_REQUEST['noticeid']?>" />
</form>	
...
<div class="arif_shadow" style="width:92%;border:solid #CCCCCC thin;border-radius:5px;margin-top:25px;margin-left:25px;padding:10px;line-height:2.0em;color:#000000;background-color:#FBFBFB;height:auto">
	...
	<span class="xlarge b"><?=$rec_guide['notice_title']?></span>
	<br />
<span style="font-weight:bold;line-height:1.4em;font-size:10px;font-family:Arial, Helvetica, sans-serif; color:#919191">	<?=date("d F Y",strtotime($rec_guide['notice_date']))?></span>

	<span class="i"><?=$rec_guide['notice_msg']?></span><?php /*?> ...<a href="guide-page.php?guide=<?=$rec_guide['notice_msg']?>" style="color:#FF9900">Read</a><?php */?>
	
	<span style="font-weight:bold;margin-top:5px;line-height:1.4em">
	<?=ucfirst(db_scalar("select reg_name  from tbl_registration where 1 and reg_id='$rec_guide[notice_from]'"))?>
	
</span>
<span style="font-weight:bold;line-height:1.4em;font-size:10px;font-family:Arial, Helvetica, sans-serif; color:#919191;float:left;"> <?=ucfirst($rec_guide['notice_from'])?>	
</span>
<br />
...
<? }else{?>
	...
	$id=db_scalar("select emp_id from tbl_emp where 1 and emp_reg_id='$_SESSION[UID_EMS]'");
		...
		if(isset($_REQUEST['more'])){
	     ...
	     if($_REQUEST['more']=="Read" || $_REQUEST['more']=="Unread"){
		   	...
		   	$sql="update tbl_notice set notice_msg_status='$_REQUEST[more]' where notice_id in ($msg_ids)";
	        db_query($sql);
			header("location:notice-board-member.php?pgid=mi&mty=in");
			exit;
			}
		     ...
		     if($_REQUEST['more']=="Yes" || $_REQUEST['more']=="No"){
		   	...
		   	$sql="update tbl_notice set notice_is_imp='$_REQUEST[more]' where notice_id in ($msg_ids)";
	        db_query($sql);
			header("location:notice-board-member.php?pgid=mi&mty=in");
			exit;
			}	
	...
	$sql="update tbl_notice set notice_is_delete='No' where notice_id in ($msg_ids)";
	db_query($sql);
	header("location:notice-board-member.php?pgid=mi&mty=tr");
	exit;
	}
	
	
	if(isset($_REQUEST['MarkReadToUnread'])){
	$sql="update tbl_notice set notice_msg_status='Unread' where notice_id in ($msg_ids)";
	db_query($sql);
	header("location:notice-board-member.php?pgid=mi&mty=Read");
	exit;
	}
	
	
	if(isset($_REQUEST['MarkUnreadToRead'])){
	$sql="update tbl_notice set notice_msg_status='Read' where notice_id in ($msg_ids)";
	db_query($sql);
	header("location:notice-board-member.php?pgid=mi&mty=Unread");
	exit;
	}
	...
	if(isset($_REQUEST['Delete'])){
		...
		$flag=0;
		if($_REQUEST['mty']!="tr"){
		$sql="update tbl_notice set notice_is_delete='Yes' where 1 and notice_id in ($msg_ids)";
		$flag=1;
		}else if($_REQUEST['mty']=="tr"){
		$sql="delete from tbl_notice where 1 and notice_id in ($msg_ids)";
		$flag=2;
		}
		...
		header("location:notice-board-member.php?pgid=mi&mty=$_REQUEST[mty]");
		}else if($flag==2){
   ...
   if($_REQUEST['mty']=='in'){
    $sql="select * from  tbl_notice where 1 and notice_to_id='$id' and notice_is_delete='No'  order by notice_date desc";
	$section="Inbox";
	}
	
    if($_REQUEST['mty']=='tr'){
    $sql="select * from  tbl_notice where 1 and notice_to_id='$id' and notice_is_delete='Yes' order by notice_date desc";
	$section="Trash";
	}
	
	if($_REQUEST['mty']=='Read'){
    $sql="select * from  tbl_notice where 1 and notice_to_id='$id' and notice_msg_status='Read'  and notice_is_delete='No'   order by notice_date desc";
	$section="Read";
	}
	
	if($_REQUEST['mty']=='Unread'){
    $sql="select * from  tbl_notice where 1 and notice_to_id='$id' and notice_msg_status='Unread'  and notice_is_delete='No'   order by notice_date desc";
	$section="Unread";
	}
	...
	<form action="" name="frm_notice" method="post" enctype="multipart/form-data" >
...
?>                  
<div id="myDiv">                                    
<div style="padding-top:10px" class="bdrAll <?php if($rec_guide['notice_msg_status']=='Unread'){?> b<?php }?>" onmouseover="this.style.background='#f3edea'" onmouseout="this.style.background=''" >
<div class="ml15px fl " style="width:20px;"><input name="arr_ids[]" type="checkbox" id="arr_ids[]" value="<?=$rec_guide['notice_id']?>" /></div>

<div class="fl ml10px" style="width:130px;font-size:11px;">
<div style="height:9px;width:16px;<? if($rec_guide['notice_is_imp']=='Yes'){?>background-color:#FFCC00<? }?>;border:#FF9900 solid thin;border-radius:0px 7px 7px 0px;float:left;margin-top:1px" onclick="change_on_click(<?=$rec_guide['notice_id']?>)">&nbsp;</div>
<a href="notice-board-member.php?pgid=mi&mty=in&noticeid=<?=$rec_guide['notice_id']?>&rd_st=R" style="color:#333333;"><p style="margin-left:22px"  ><?=$rec_guide['notice_by']?></p></div>
<div class="fl ml10px" style="width:340px;font-size:11px;"><p >
<?
if(!empty($rec_guide['notice_title'])){
echo substr($rec_guide['notice_title'],0,40);
if(strlen($rec_guide['notice_title'])>40){
echo "...";
}
...
?></p></div>
<div class="fl ml10px" style="width:150px;font-size:11px;"><p style="padding-left:60px" ><?=date("d M Y",strtotime($rec_guide['notice_date']))?></p></div></a>
<p class="cb"></p>
...
document.frm_notice.submit();
}


function change_on_click(nid){
window.location="notice-board-member.php?nid="+nid
}
</script>
