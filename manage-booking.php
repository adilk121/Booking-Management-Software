...
$sear_by=$_REQUEST['search_by'];


if($sear_by=="document")
{
	$cond="and shipment_type='$sear_by' ";
}

if($sear_by=="non_document")
{
	$cond="and shipment_type='$sear_by' ";
}


if($sear_by=="docket_num")
{


	$d_no=$_REQUEST['docket_no'];

	$cond="and docket_number='$d_no' ";
...
<p class=" xlarge mt10px ml10px"><span class="b " style="font-size:12px"><i class="fa fa-list"></i> Manage Booking &raquo; <span style="color:#999999"><?/* if(!empty($_REQUEST['state'])){?>
<?=ucfirst(strtolower($_REQUEST['state']))?><? }?> Employee</span></span>
*/?>

<!-- <select name="search_by" onchange='CheckSearch(this.value);' style="padding:3px;"> 
    <option>Search By</option>  
<option value="docket_num">Docket Number</option>
<option value="document">Document</option>
<option value="non_document">Non Document</option>

  </select>-->

<input type="text" name="docket_no" id="docket_no" placeholder="Enter Docket Number" style='display:none;'/>
...
function CheckSearch(val){
 var rec=document.getElementById('docket_no');
 if(val=='docket_num')
 {
...
<a href="add-booking.php" class="btn btn-default pull-right mr15px font-black"><i class="fa fa-plus"></i> Add Booking</a>
...
<form action="" method="post" enctype="multipart/form-data" >
	...
	<td style="width:70px;"><p class="b p10px blue ac">Airway Bill Number	</p></td>
	
	
	<td style="width:70px;"><p class="b p10px blue ac">Shipper Name/Code	</p></td>

	<td style="width:100px;"><p class="b p10px blue ac">Transit Mode</p></td>
	<td style="width:9px;"><p class="b p10px blue ac">Type N/V</p></td>
	<td  style="width:100px;"><p class="b p10px blue ac">No. of Package</p></td>

	<td  style="width:30px;"><p class="b p10px blue ac">Booking Date</p></td>
...
<?php
 ...
 $i = 0; 
$sql=db_query("select * from tbl_package where 1 order by package_id desc");
...
<tr>
		...
		<td style="width:100px;"><p class=" p10px  ac">
		    <a style="text-decoration: none; color: #bf0000;" href='#'>
		    <strong>
		        <?=$res['airway_bill_no']?>
		    </strong>
		    </p>
		...
		<td style="width:100px;">
		    <p class=" p10px  ac">
		    <?=$res['shipper_name']?> / <?=db_scalar("select shipper_code from tbl_shipper where shipper_id='$res[shipper_id]'");?>
		   </p>
		   </td>
		<td style="width:100px;"><p class=" p10px  ac"><?=$res['transit_mode']?></p></td>
		<td style="width:100px;"><p class=" p10px  ac"><?=$res['type_nv']?></p></td>
		<td style="width:100px;"><p class=" p10px  ac"><?=$res['no_of_package']?></p></td>
		<td style="width:100px;"><p class=" p10px  ac"><?=$res['booking_date']?></p></td>
		<td style="width:100px;"><p class=" p10px  ac"><a href="add-booking.php?pack_id=<?=$res[package_id]?>" style="color:green; font-weight:bold;">Edit</a> 
		 <!--<a href="manage-booking.php?dock_no=<?=$res['docket_number']?>" style="color:red; font-weight:bold;">Delete</a>--></p></td>
...
<a href="edit-employe.php?empid=<?=$record_emp['emp_reg_id']?>"><img src="images/edit.gif" /></a>

<a href="../html-link.htm" target="popup" onclick="window.open('../html-link.htm','name','width=1000,height=800')">
<img src="images/edit.gif" style="background-color:red"/> 
</a>
