...
window.location="manage-offers.php?status=" + val;
}
</script>
...
$arr_ids = $_REQUEST['arr_ids'];
	if(is_array($arr_ids)) {
		...
		if(isset($_REQUEST['Active'])) {
			db_query("update  tbl_offer set offer_status = 'Active' where offer_id in ($str_ids)");
			$c=mysql_affected_rows();
			$_SESSION['msg']="$c Record(s) are activated";
		} else if(isset($_REQUEST['Inctive']) ){
			db_query("update  tbl_offer set offer_status = 'Inactive' where offer_id in ($str_ids)");
			$c=mysql_affected_rows();
			$_SESSION['msg']="$c Record(s) are deactivated";
		}else if(isset($_REQUEST['delete']) ){
		
			db_query("delete from tbl_offer where offer_id in ($str_ids)");
			$c=mysql_affected_rows();
...
<p class=" xlarge mt10px ml10px"><span class="b"><i class="fa fa-star" aria-hidden="true"></i> Manage Offers</span>
</p>
...
$cond="and  offer_status ='$_REQUEST[status]' ";
}

$sql="select * from tbl_offer where 1 $cond";

$start = intval($_REQUEST['start']);
$pagesize = intval($pagesize)==0?$pagesize=DEF_PAGE_SIZE:$pagesize;
$order_by == '' ? $order_by = 'offer_date' : true;
...
<?=pagesize_dropdown('pagesize', $pagesize);?></span>
	...
	<td width="17%"><p class="b p10px blue ac">To</p></td>	
	<td width="25%"><p class="b p10px blue ac">Detail</p></td>

	<td width="15%"><p class="b p10px blue ac">Status / Date</p></td>
...
?>
	...
	<tr class="<? if($record_news['offer_msg_status']=="Unread" and $record_news['offer_type']!="dept"){?> b<? }?>">
	<td width="3%" align="center"><?=$i?></td>
	<td style="width:100px;" align="left"> <p class=" p10px u  al ml20px" title="Click here to view detail" >
	<a class="pop4" style="color:#0000CC" href="offer_detail.php?offer=<?=$record_news['offer_id']?>"  >
	<?=ucfirst($record_news['offer_title'])?></a></p>		
	</td>
	
<td style="width:100px;"><p class=" p10px black al vl ml20px">

<?=$record_news['offer_to']?>
</p></td>
	
<td style="width:100px;vertical-align:top;padding-left:10px;"><p class=" p10px black al" ><?=substr($record_news['offer_desc'],0,40)?></p></td>
...
<p class="p10px <?php if($record_news['offer_status']=="Active"){?>green<? }else{?>red<?php }?> ac">

<?=$record_news['offer_status']?> &nbsp;&nbsp; <span style="color:#000000;">[<?=date("d-M-Y",strtotime($record_news['offer_date']))?>]</span>
...
<td style="width:30px;">
<p class=" p10px  ac"><a href="add-edit-offers.php?offer=<?=$record_news['offer_id']?>"><img src="images/edit.gif" /></a></p>
</td>
...
<input name="arr_ids[]" type="checkbox" id="arr_ids[]" value="<?=$record_news['offer_id']?>"  />  
...
<tr>
<td colspan="12">
<p class="p10px fl"><a href="add-edit-offers.php"><input type="button" value="ADD OFFER" style="width:180px; height:28px;font-weight:700;"/></a></p>
