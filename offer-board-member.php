...
<p style="color:#333333;font-size:16px;font-weight:bold;margin-left:25px;margin-top:10px;">My Offers

<span style="color:#666666;font-size:12px;font-weight:bold;margin-right:25px;margin-top:10px;float:right"><a href="home-page.php" style="color:#999999">Home</a> &raquo My Offers</span>
</p>
	...
	$curr_state=db_scalar("select offer_is_imp from tbl_offer where 1 and offer_id='$_REQUEST[oid]'");
	
	if($curr_state=="Yes"){
	db_query("update tbl_offer set offer_is_imp='No' where 1 and offer_id='$_REQUEST[oid]'");	
	}else{
	db_query("update tbl_offer  set offer_is_imp='Yes' where 1 and offer_id='$_REQUEST[oid]'");	
	}
    

	
	
	header("location:offer-board-member.php?pgid=mi&mty=in");
	exit;	
	}
	if(isset($_REQUEST['MakeUnread'])){
	db_query("update tbl_offer set offer_msg_status='Unread' where 1 and offer_id='$_REQUEST[offerid]'");	
	header("location:offer-board-member.php?pgid=mi&mty=in");
	exit;
	}
	
	if(isset($_REQUEST['DetailDelete'])){
	db_query("update tbl_offer set offer_is_delete='Yes' where 1 and offer_id='$_REQUEST[offerid]'");	
	header("location:offer-board-member.php?pgid=mi&mty=in");
	exit;
	}
	...
	db_query("update tbl_offer set offer_msg_status='Read' where 1 and offer_id='$_REQUEST[offerid]'");
	
	$sql="select * from  tbl_offer where 1 and offer_id='$_REQUEST[offerid]'";
	$rec_guide=mysql_fetch_array(db_query($sql));	
	
	?>
<form action="" method="post" >
<div> <a href="offer-board-member.php?pgid=mi&mty=in">
...
<input type="hidden" name="offer_id_h" value="<?=$_REQUEST['offerid']?>" />
</form>	
...
<div class="arif_shadow" style="width:92%;border:solid #CCCCCC thin;border-radius:5px;margin-top:25px;margin-left:25px;padding:10px;line-height:2.0em;color:#000000;background-color:#FBFBFB;height:auto">
	...
	<span class="xlarge b"><?=$rec_guide['offer_title']?></span>
	<br />
<span style="font-weight:bold;line-height:1.4em;font-size:10px;font-family:Arial, Helvetica, sans-serif; color:#919191">	<?=date("d F Y",strtotime($rec_guide['offer_date']))?></span>

	<span class="i"><?=$rec_guide['offer_msg']?></span><?php /*?> ...<a href="guide-page.php?guide=<?=$rec_guide['offer_msg']?>" style="color:#FF9900">Read</a><?php */?>
	
	<span style="font-weight:bold;margin-top:5px;line-height:1.4em">
	<?=ucfirst(db_scalar("select reg_name  from tbl_registration where 1 and reg_id='$rec_guide[offer_from]'"))?>
	
</span>
<span style="font-weight:bold;line-height:1.4em;font-size:10px;font-family:Arial, Helvetica, sans-serif; color:#919191;float:left;"> <?=ucfirst($rec_guide['offer_from'])?>	
</span>
<br />
...
<? }else{?>
	...
	if(isset($_REQUEST['more'])){
	     ...
	     if($_REQUEST['more']=="Read" || $_REQUEST['more']=="Unread"){
		   	...
		   	$sql="update tbl_offer set offer_msg_status='$_REQUEST[more]' where offer_id in ($msg_ids)";
	        db_query($sql);
			header("location:offer-board-member.php?pgid=mi&mty=in");
			exit;
			}
		     ...
		     if($_REQUEST['more']=="Yes" || $_REQUEST['more']=="No"){
		   	...
		   	$sql="update tbl_offer set offer_is_imp='$_REQUEST[more]' where offer_id in ($msg_ids)";
	        db_query($sql);
			header("location:offer-board-member.php?pgid=mi&mty=in");
			exit;
			}	
	...
	$sql="update tbl_offer set offer_is_delete='No' where offer_id in ($msg_ids)";
	db_query($sql);
	header("location:offer-board-member.php?pgid=mi&mty=tr");
	exit;
	}
	
	
	if(isset($_REQUEST['MarkReadToUnread'])){
	$sql="update tbl_offer set offer_msg_status='Unread' where offer_id in ($msg_ids)";
	db_query($sql);
	header("location:offer-board-member.php?pgid=mi&mty=Read");
	exit;
	}
	
	
	if(isset($_REQUEST['MarkUnreadToRead'])){
	$sql="update tbl_offer set offer_msg_status='Read' where offer_id in ($msg_ids)";
	db_query($sql);
	header("location:offer-board-member.php?pgid=mi&mty=Unread");
	exit;
	}
	...
	if(isset($_REQUEST['Delete'])){
		...
		$flag=0;
		if($_REQUEST['mty']!="tr"){
		$sql="update tbl_offer set offer_is_delete='Yes' where 1 and offer_id in ($msg_ids)";
		$flag=1;
		}else if($_REQUEST['mty']=="tr"){
		$sql="delete from tbl_offer where 1 and offer_id in ($msg_ids)";
		$flag=2;
		}
		...
		header("location:offer-board-member.php?pgid=mi&mty=$_REQUEST[mty]");
		}else if($flag==2){
   ...
   if($_REQUEST['mty']=='in'){
    $sql="select * from  tbl_offer where 1 and offer_to_id='$id' and offer_is_delete='No'  order by offer_date desc";
	$section="Inbox";
	}
	
    if($_REQUEST['mty']=='tr'){
    $sql="select * from  tbl_offer where 1 and offer_to_id='$id' and offer_is_delete='Yes' order by offer_date desc";
	$section="Trash";
	}
	
	if($_REQUEST['mty']=='Read'){
    $sql="select * from  tbl_offer where 1 and offer_to_id='$id' and offer_msg_status='Read'  and offer_is_delete='No'   order by offer_date desc";
	$section="Read";
	}
	
	if($_REQUEST['mty']=='Unread'){
    $sql="select * from  tbl_offer where 1 and offer_to_id='$id' and offer_msg_status='Unread'  and offer_is_delete='No'   order by offer_date desc";
	$section="Unread";
	}
	...
	<form action="" name="frm_offer" method="post" enctype="multipart/form-data" >
...
?>                  
<div id="myDiv">                                    
<div style="padding-top:10px" class="bdrAll <?php if($rec_guide['offer_msg_status']=='Unread'){?> b<?php }?>" onmouseover="this.style.background='#f3edea'" onmouseout="this.style.background=''" >
<div class="ml15px fl " style="width:20px;"><input name="arr_ids[]" type="checkbox" id="arr_ids[]" value="<?=$rec_guide['offer_id']?>" /></div>

<div class="fl ml10px" style="width:130px;font-size:11px;">
<div style="height:9px;width:16px;<? if($rec_guide['offer_is_imp']=='Yes'){?>background-color:#FFCC00<? }?>;border:#FF9900 solid thin;border-radius:0px 7px 7px 0px;float:left;margin-top:1px" onclick="change_on_click(<?=$rec_guide['offer_id']?>)">&nbsp;</div>
<a href="offer-board-member.php?pgid=mi&mty=in&offerid=<?=$rec_guide['offer_id']?>&rd_st=R" style="color:#333333;">
<p style="margin-left:22px" >Director</p></div>
<div class="fl ml10px" style="width:340px;font-size:11px;"><p >
<?
if(!empty($rec_guide['offer_title'])){
echo substr($rec_guide['offer_title'],0,40);
if(strlen($rec_guide['offer_title'])>40){
echo "...";
}
...
?></p></div>
<div class="fl ml10px" style="width:150px;font-size:11px;"><p style="padding-left:60px" ><?=date("d M Y",strtotime($rec_guide['offer_date']))?></p></div></a>
<p class="cb"></p>
...
document.frm_offer.submit();
}


function change_on_click(oid){
window.location="offer-board-member.php?oid="+oid
}
</script>
</body></html>